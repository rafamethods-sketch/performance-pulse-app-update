...
function buildCoachAttentionItems(clients: CoachClient[], period: AttentionPeriod): CoachAttentionItem[] {
  ...
  clients.forEach((client) => {
    ...
    sessionRecords.forEach((session, sessionIndex) => {
      ...
      const reviewSession = session as ReviewSessionRecord;

      if (getSessionReviewStatus(reviewSession) === "pending") {
        const srpe = getSessionSrpe(reviewSession);
        items.push({
          action: "session",
    ...
    if (!onboardingCompletion.isComplete) {
      ...
      items.push({
        ...
        meta: onboardingSummary.primarySport || client.modality || client.sport,
...
function CoachAnalyticsView({ clients }: { clients: CoachClient[] }) {
